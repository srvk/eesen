...
void Nnet::WriteNonParal(const std::string &file, bool binary) const {
  Output out(file, binary, true);
...
void Nnet::WriteNonParal(std::ostream &os, bool binary) const {
  Check();
  WriteToken(os, binary, "<Nnet>");
  if(binary == false) os << std::endl;
  for(int32 i=0; i<NumComponents(); i++) {
    components_[i]->WriteNonParal(os, binary);
  }
  WriteToken(os, binary, "</Nnet>");
  if(binary == false) os << std::endl;
}
