...
num_sequence=10           # during training, how many utterances to be processed in parallel
valid_num_sequence=20    # number of parallel sequences in validation
frame_num_limit=12500  # the number of frames to be processed at a time in training; this config acts to
...
end_halving_inc=0.01      # terminate training when the accuracy improvement falls below this amount
halving_factor=0.5       # learning rate decay factor
halving_after_epoch=1    # halving bcomes enabled after this many epochs

# logging
report_step=1          # during training, the step (number of utterances) of reporting objective and accuracy
verbose=1
...
output_validation_file=output_validation_sequence
...
if [ ! -f $dir/nnet/nnet.iter0 ]; then
    ...
    net-initialize --binary=false $dir/nnet.proto $dir/nnet/nnet.iter0 >& $dir/log/initialize_model.log || exit 1;
fi
...
for iter in $(seq $start_epoch_num $max_iters); do
    ...
    # train
    $train_tool --binary=false --report-step=$report_step --num-sequence=$num_sequence --frame-limit=$frame_num_limit \
