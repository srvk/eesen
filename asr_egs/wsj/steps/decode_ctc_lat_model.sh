...
model=$4; # don't assume model directory one level up from decoding directory.
sdata=$data/split$nj;

thread_string=
[ $num_threads -gt 1 ] && thread_string="-parallel --num-threads=$num_threads"

[ -z "$add_deltas" ] && add_deltas=`cat $model/add_deltas 2>/dev/null`
[ -z "$norm_vars" ] && norm_vars=`cat $model/norm_vars 2>/dev/null`
...
for f in $graphdir/TLG.fst $model/label.counts $data/feats.scp; do
...
$cmd JOB=1:$nj $dir/log/decode.JOB.log \
  net-output-extract --class-frame-counts=$model/label.counts --apply-log=true $model/final.nnet "$feats" ark:- \| \
