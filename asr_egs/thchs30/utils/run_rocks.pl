...
for ($jobid = $jobstart; $jobid <= $jobend; $jobid++) {
  ...
  if ($childpid == 0) { # We're in the child... this branch
    ...
    $cmdid = $jobid - $jobstart + 1;
    $cmdfile = "./cmds/cmd." . $cmdid;
    $donefile = "./dones/done." . $cmdid;
    unlink $donefile;
    open(F, ">$cmdfile") || die "Error opening cmd file $cmdfile";
#    print F "( " . $cmd . ") 2>>$logfile >> $logfile";
    print F "" . $cmd . " 2>> $logfile\n";
    close(F);
