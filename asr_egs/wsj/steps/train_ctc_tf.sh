...
cp $dir/labels.tr $tmpdir/labels.tr || exit 1
cp $dir/labels.cv $tmpdir/labels.cv || exit 1

echo ""
echo cleaning train set ...
echo ""

python ./utils/clean_length.py --scp_in  $tmpdir/train_local.scp --labels $tmpdir/labels.tr --subsampling 3 --scp_out $tmpdir/train_local.scp
