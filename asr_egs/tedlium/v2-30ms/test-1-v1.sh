...
. ./utils/parse_options.sh
    ...
    dir=exp/train_phn_l${lstm_layer_num}_c${lstm_cell_dim}_v1s
    tst=$dir/decode_dev_v1

    steps/decode_ctc_lat.sh --cmd "$decode_cmd" --nj 8 --beam 17.0 --lattice_beam 8.0 --max-active 5000 --acwt 0.6 \
        data/lang_phn_test data/dev $tst

    [ -f data/dev/glm ] || (cd data/dev; ln -s /data/ASR4/babel/cmilo/sctk-2.4.8/src/test_suite/example.glm glm)
    local/score_sclite.sh --cmd run.pl --min_acwt 1 --max_acwt 9  data/dev data/lang_phn_test $tst
