...
def train(all_readers, config):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(alpha, config[lm_constants.CONF_TAGS.NEPOCH]):
            ...
            train_cost, train_ppl, ntrain, ntrain_w = train_epoch(sess, model, config, data_queue, epoch, lr_rate, tr_x, tr_sat)

            cv_cost, cv_ppl, ncv, ncv_w = eval_epoch(sess, model, config, data_queue, cv_x, cv_sat)

            saver.save(sess, "%s/epoch%02d.ckpt" % (config[lm_constants.CONF_TAGS.TRAIN_DIR], epoch))

            generate_logs(config, cv_cost, cv_ppl, ncv, ncv_w, train_cost, train_ppl, ntrain, ntrain_w, epoch, lr_rate, time.time())
...
def train_epoch(sess, model, config, data_queue, epoch,  lr_rate, tr_x, tr_sat):
    ...
    if(tr_sat):
        p = Process(target = run_reader_queue, args = (data_queue, tr_x, config[lm_constants.CONF_TAGS.DEBUG], config[lm_constants.CONF_TAGS.DO_SHUF]))
    else:
    ...
    train_cost, train_ppl, ntrain, ntrain_w = 0.0, 0.0, 0, 0
    ...
    while True:
        ...
        batch_cost, batch_ppl, _ = sess.run([model.loss, model.ppl, model.optimizer], feed_dict = feed_dict)

        train_cost, train_ppl, ntrain, ntrain_w = update_counters(train_cost, train_ppl, ntrain, ntrain_w, batch_cost, batch_ppl, batch_len)

    p.join()
    p.terminate()

    return train_cost, train_ppl, ntrain, ntrain_w

def eval_epoch(sess, model, config, data_queue, cv_x, cv_sat):

    cv_cost, cv_ppl, ncv, ncv_w = 0.0, 0.0, 0, 0

    if(cv_sat):
        p = Process(target = run_reader_queue, args = (data_queue, cv_x, config[lm_constants.CONF_TAGS.DEBUG], config[lm_constants.CONF_TAGS.DO_SHUF]))
    else:
    ...
    while True:
        ...
        batch_cost, batch_ppl = sess.run([model.loss, model.ppl], feed_dict = feed_dict)

        ncurrent_words = sum(batch_len) - len(batch_len)

        cv_cost, cv_ppl, ncv, ncv_w, = update_counters(cv_cost, cv_ppl, ncv, ncv_w, batch_cost, batch_ppl, batch_len)

    return cv_cost, cv_ppl, ncv, ncv_w

#self.__update_counters(train_ters, train_cost, ntrain, ntr_labels, batch_ters, batch_cost, batch_size, data[1])
def update_counters(acum_cost, acum_ppl, acum_utt, acum_words, batch_cost, batch_ppl, batch_len):
    ...
    return acum_cost, acum_ppl, acum_utt, acum_words
...
def generate_logs(config, cv_cost, cv_ppl, ncv, ncv_w, train_cost, ntrain, train_ppl, ntrain_w, epoch, lr_rate, tic):
