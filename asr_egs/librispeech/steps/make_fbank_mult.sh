...
#           2017  Jayadev Billa - adaptations for generating max perturbation features
...
if [ -f $data/spk2warp ]; then
  ...
  echo "$0 [info]: using VTLN warp factor as $vtln_warp "
  vtln_opts="--vtln-warp=$vtln_warp"
fi
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_fbank_${name}_${tag}.JOB.log \
    extract-segments scp,p:$scp $logdir/segments.JOB ark:- \| \
    compute-fbank-feats $vtln_opts --verbose=2 --config=$fbank_config ark:- ark:- \| \
    copy-feats --compress=$compress ark:- \
     ark,scp:$fbankdir/raw_fbank_${name}_${tag}.JOB.ark,$fbankdir/raw_fbank_${name}_${tag}.JOB.scp \
     || exit 1;
...
if [ -f $logdir/.error.$name ]; then
  ...
  tail $logdir/make_fbank_${name}_${tag}.1.log
  exit 1;
fi

# concatenate the .scp files together.
for n in $(seq $nj); do
  cat $fbankdir/raw_fbank_${name}_${tag}.$n.scp || exit 1;
done > $data/feats_${tag}.scp
