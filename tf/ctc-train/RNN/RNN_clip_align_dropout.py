...
class RNN(object):
    def __init__(self,
        ...
        if initParams is not None:          # load parameters from given dict
            ...
            self.paramNames = ["Nlayers", "Ndirs", "Nx", "Nh", "Ny", "Ah", "Ay", "predictPer", "loss", "L1reg", "L2reg", "dropout", "momentum", "frontEnd"]
        ...
        def forward_pass(x, dropout):
            if dropout != 0.0:
                x *= theano_rng.binomial(n = 1, p = 1 - dropout, size = x.shape, dtype = theano.config.floatX) / (1 - dropout)
            for i in range(Nlayers):
                ...
                if dropout != 0.0:
                    h *= theano_rng.binomial(n = 1, p = 1 - dropout, size = h.shape, dtype = theano.config.floatX) / (1 - dropout)
