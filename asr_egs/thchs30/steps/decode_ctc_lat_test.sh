...
feats="ark,s,cs:apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$sdata/1/utt2spk scp:$sdata/1/cmvn.scp scp:$sdata/1/feats.scp ark:- |"
...
#$cmd $dir/log/decode.1.log \ 
net-output-extract --class-frame-counts=$srcdir/label.counts --apply-log=true --use-gpu="yes" $srcdir/$mdl "$feats" ark,t:output.txt
exit 1;
# Decode for each of the acoustic scales
$cmd JOB=1:$nj $dir/log/decode.JOB.log \
  net-output-extract --class-frame-counts=$srcdir/label.counts --apply-log=true --use-gpu="yes" $srcdir/$mdl "$feats" ark:- \| \
...
# Scoring
if  [ ! -x local/score.sh ] && echo "Not scoring because local/score.sh does not exist or not executable." && exit 1;
