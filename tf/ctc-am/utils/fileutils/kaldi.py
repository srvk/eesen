...
if not 'KALDI_ROOT' in os.environ:
  ...
  os.environ['KALDI_ROOT']='/data/ASR5/xinjianl/Git/kaldi'
...
def open_or_fd(file, mode='rb', info=False):
  """ fd = open_or_fd(file)
...
def read_scp_info_dic(filename, limit = np.inf):
    pairs = read_mat_scp(filename, info=True)
    res = {}
    for utt, arkfile, offset, info in pairs:
        feat_len = info[0]
        feat_dim = info[1]
        res[utt]=((utt, arkfile, offset, feat_len, feat_dim))
...
def read_scp_info(filename, limit = np.inf):
    ...
    for utt, arkfile, offset, info in pairs:
        feat_len = info[0]
        feat_dim = info[1]
        res.append((utt, arkfile, offset, feat_len, feat_dim))
        if len(res) == limit: break
    return res
...
def readMatrixByOffset(arkfile, offset):
    ...
    binary = fd.read(2).decode()
    if binary == '\0B' :
        feature = _read_mat_binary(fd)
    return feature

# Reading,
def read_mat_scp(file_or_fd, info=False):
  ...
  try:
    for line in fd:
      (key,rxfile) = line.decode().split(' ')
      if info:
        arkfile, offset, mat = read_mat(rxfile,info)
        yield key, arkfile, offset, mat
      else:
        mat = read_mat(rxfile)
        yield key, mat
  finally:
...
def read_mat(file_or_fd, info=False):
  """ [mat] = read_mat(file_or_fd)
   Reads single kaldi matrix, supports ascii and binary.
   file_or_fd : file, gzipped file, pipe or opened file descriptor.
  """
  if (info):
      fd, arkfile,offset = open_or_fd(file_or_fd, info=True)
  else:
      fd = open_or_fd(file_or_fd)
  ...
  try:
    ...
    if fd is not file_or_fd: fd.close()
  if (info):
    return arkfile, offset, mat
  return mat

def _read_mat_binary(fd, info=False):
  # Data type
  header = fd.read(3).decode()
  # 'CM', 'CM2', 'CM3' are possible values,
  if header.startswith('CM'):
      return _read_compressed_mat(fd, header, info)
...
def _read_compressed_mat(fd, format, info=False):
...
def writeArk(filename, features, uttids):
    ...
    fd = open_or_fd(filename, mode='ab+')
    pointers = []
    for idx in range(len(uttids)):
        utt = uttids[idx]
        feat = features[idx]
        fd.write((utt + ' ').encode())
        pointers.append("%s:%d" % (filename, fd.tell()))

        fd.write('\0B'.encode()) # we write binary!
        # Data-type,
        if feat.dtype == 'float32': fd.write('FM '.encode())
        elif feat.dtype == 'float64': fd.write('DM '.encode())
        else: raise UnsupportedDataType("'%s', please use 'float32' or 'float64'" % feat.dtype)
        # Dims,
        fd.write('\04'.encode())
        fd.write(struct.pack(np.dtype('uint32').char, feat.shape[0])) # rows
        fd.write('\04'.encode())
        fd.write(struct.pack(np.dtype('uint32').char, feat.shape[1])) # cols
        # Data,
        fd.write(feat.tobytes())
