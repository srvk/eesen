...
# Source: https://github.com/vesis84/kaldi-io-for-python
...
def write_mat(file_or_fd, m, key=''):
    ...
    fd = open_or_fd(file_or_fd, mode='wb')
    try:
        if key != '': fd.write(bytearray(key + ' ', encoding="utf8"))  # ark-files have keys (utterance-id),
