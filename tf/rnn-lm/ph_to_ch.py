...
test_file = 'dev.txt'
ch2i = defaultdict(count(0).next)
sos = '<s>'
eos = '<s>'
space = ' '
train_fil = open('train.txt').readlines()
dev_fil = open('dev.txt').readlines()
units_ch = open('units_char.txt').readlines()
ch2i['<s>']
for i in units_ch:
    ch2i[i.strip().split()[0]]
# w2i[eos]
ch2i[space]

lexicon_ch = open('lexicon_numbers_char.txt').readlines()
lexicon_dict_ch = {}
for i in lexicon_ch:
    lexicon_dict_ch[i.strip().split()[0]] = i.strip().split()[1:]
train_list = []
...
for tr in train_fil:
    ...
    for w in words:
        for ch in lexicon_dict_ch[w]:
            temp.append(int(ch))
        temp.append(ch2i[space])
    temp.append(ch2i['<s>'])
