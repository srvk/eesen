...
// <jiayu>
//#include "cudamatrix/bd-cu-kernels.h"
// </jiayu>
...
template<typename Real>
void CuMatrixBase<Real>::ComputeCtcErrorRescale(const CuMatrixBase<Real> &alpha,
                                         const CuMatrixBase<Real> &beta,
                                         const CuMatrixBase<Real> &prob,
                                         const std::vector<int32> &labels,
...
#if HAVE_CUDA == 1
  if (CuDevice::Instantiate().Enabled()) {
    KALDI_ASSERT(alpha.NumRows() == NumRows() && beta.NumRows() == NumRows() && prob.NumRows() == NumRows() && zt.Dim() == NumRows());
    KALDI_ASSERT(alpha.NumCols() == beta.NumCols());
    KALDI_ASSERT(prob.NumCols() == NumCols());
    KALDI_ASSERT(static_cast<MatrixIndexT>(labels.size()) == alpha.NumCols());
#ifdef KALDI_PARANOID
    ...
    dim3 dimBlock(CU2DBLOCK, CU2DBLOCK);
    dim3 dimGrid(n_blocks(NumRows(), CU2DBLOCK), n_blocks(NumCols(), CU2DBLOCK));
    cuda_compute_ctc_error_rescale(dimGrid, dimBlock, data_, Dim(), alpha.data_, beta.data_, alpha.Dim(), prob.data_, cuda_labels.Data(), zt.data_);
    CU_SAFE_CALL(cudaGetLastError());
...
template<typename Real>
void CuMatrixBase<Real>::DistributeProbByLabel(const CuMatrixBase<Real> &prob,
                                               const std::vector<int32> &labels) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    dim3 dimBlock(CU2DBLOCK, CU2DBLOCK);
    dim3 dimGrid(n_blocks(NumRows(), CU2DBLOCK), n_blocks(NumCols(), CU2DBLOCK));
    cuda_distribute_prob_by_label(dimGrid, dimBlock, data_, Dim(), prob.data_, prob.Dim(), cuda_labels.Data());
    CU_SAFE_CALL(cudaGetLastError());
