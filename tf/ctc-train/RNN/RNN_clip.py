...
from ctc import ctc_cost
import sys, os.path
sys.path.append(os.path.expanduser("/home/yunwang/G/coconut"))
...
class RNN(object):
    def __init__(self,
        ...
        if initParams is not None:          # load parameters from given dict
            ...
            Nlayers, Ndirs, Nx, Nh, Ny, Nproj, Ah, Ay, predictPer, loss, L1reg, L2reg, momentum, frontEnd \
                = self.Nlayers, self.Ndirs, self.Nx, self.Nh, self.Ny, self.Nproj, self.Ah, self.Ay, self.predictPer, self.loss, self.L1reg, self.L2reg, self.momentum, self.frontEnd
        else:                           # Initialize parameters randomly
            # Names of parameters to save to file
            self.paramNames = ["Nlayers", "Ndirs", "Nx", "Nh", "Ny", "Nproj", "Ah", "Ay", "predictPer", "loss", "L1reg", "L2reg", "momentum", "frontEnd"]
            ...
            self.addParam("Wrec", rand_init((Nlayers, Ndirs, Nproj, Nh * Nlstm), Ah))
            self.addParam("Wup", rand_init((Nlayers - 1, Nproj * Ndirs, Nh * Ndirs * Nlstm), Ah))
            self.addParam("Wproj", rand_init((Nlayers, Ndirs, Nh, Nproj), Ah))
            self.addParam("Wout", rand_init((Nproj * Ndirs, Ny), Ay))
            if Ah != "lstm":
            ...
            self.addParam("h0", zeros((Nlayers, Ndirs, Nproj)))
            if Ah == "lstm":
        ...
        mask_int = [T.eq(mask, 1).nonzero(), T.eq(mask, 2).nonzero()]
        mask_float = [T.cast(T.eq(mask, 1).dimshuffle((1, 0)).reshape((mask.shape[1], mask.shape[0], 1)), theano.config.floatX),
                      T.cast(T.eq(mask, 2).dimshuffle((1, 0)).reshape((mask.shape[1], mask.shape[0], 1)), theano.config.floatX)]

        def step_rnn(x_t, mask, h_tm1, W, h0):
            h_tm1 = T.switch(mask, h0, h_tm1)
            return [ACTIVATION[Ah](x_t + h_tm1.dot(W))]

        def step_lstm(x_t, mask, c_tm1, h_tm1, W, c0, h0, Wproj):
        ...
        for i in range(Nlayers):
            ...
            if Ah != "lstm":
                ...
                h = T.concatenate([theano.scan(
                        ...
                        non_sequences = [self.Wrec[i, d], rep(self.c0[i, d]), rep(self.h0[i, d]), self.Wproj[i, d]],
                        go_backwards = (d == 1),
