...
if [ $# != 3 ]; then
   ...
   echo "Options:"
   echo " --tag <tag>     uses tag as suffix to feats file"
...
required="$data/feats_${tag}.scp $data/spk2utt"
...
if $fake; then
  dim=`feat-to-dim scp:$data/feats_${tag}.scp -`
  ...
  ! compute-cmvn-stats-two-channel $data/reco2file_and_channel scp:$data/feats_${tag}.scp \
  ...
  ! compute-cmvn-stats --spk2utt=ark:$data/spk2utt scp:$data/feats_${tag}.scp ark:- | \
  ...
  ! compute-cmvn-stats --spk2utt=ark:$data/spk2utt scp:$data/feats_${tag}.scp ark,scp:$cmvndir/cmvn_$name.ark,$cmvndir/cmvn_$name.scp \
